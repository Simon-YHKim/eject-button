 ...
 *
 * 시간 윈도우는 호출 측에서 주입한다:
 * - Foreground (MainActivity) → 700/1000ms — 실제 키 이벤트를 직접 받으므로
 *   사람의 '빠른 더블탭/트리플탭' 감도에 맞춰 타이트하게.
 * - Background (ButtonWatchService) → 1200/1600ms — ContentObserver 를 통해
 *   입력이 전달되는 특성상 전파 지연이 있어 조금 여유있게.
 *
 * CUSTOM 은 sequenceLength * 600ms (최소 1200ms, 최대 3000ms) 로 자동 계산.
 * 윈도우 밖의 오래된 입력은 자동으로 폐기된다. 매칭 성공 시 큐를 비워 즉시
 * 재트리거되지 않도록 한다.
 */
class ButtonPatternDetector(
    ...
    private val maxWindowMs: Long = maxOf(doubleWindowMs, tripleWindowMs)
    ...
    private fun push(queue: ArrayDeque<Long>, now: Long) {
        queue.addLast(now)
        // 오래된 항목 폐기 — 최대 윈도우보다 오래된 것은 의미 없음
        while (queue.isNotEmpty() && now - queue.first() > maxWindowMs) {
            queue.removeFirst()
        }
    }
    ...
    private fun matches(queue: ArrayDeque<Long>, tapCount: Int): Boolean {
        ...
        val window = if (tapCount == 2) doubleWindowMs else tripleWindowMs
    ...
    companion object {
        // 기본값(= Background 용 완화 윈도우). Foreground 에서는 700/1000 을 명시 주입.
        const val DEFAULT_DOUBLE_WINDOW_MS = 1200L
        const val DEFAULT_TRIPLE_WINDOW_MS = 1600L
        const val FOREGROUND_DOUBLE_WINDOW_MS = 700L
        const val FOREGROUND_TRIPLE_WINDOW_MS = 1000L
